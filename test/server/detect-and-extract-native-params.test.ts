import { detectAndExtractNativeParams } from '../../src/server/detect-and-extract-native-params';
import {checkIsWebview} from "../../src/server";
...
jest.mock('../../src/server/check-is-webview');

describe('detectAndExtractNativeParams', () => {
    const mockCheckIsWebview = checkIsWebview as jest.Mock;

    beforeEach(() => {
        jest.clearAllMocks();
        mockedUa = UA_AMWEBVIEW;
        mockedExtractAppVersion = originalUtils.extractAppVersion;
        mockCheckIsWebview.mockReturnValue(true);
    });

    describe('non-webview', () => {
        beforeEach(() => {
            mockedUa = UA_IPHONE;
            mockCheckIsWebview.mockReturnValue(false);
        });
        ...
        it.each([
            ...
            expect(detectAndExtractNativeParams(request)).toEqual({
                isWebview: false,
            });
        });
    });
    ...
    describe('isWebview', () => {
        ...
        it('should pass non AM-webview env', () => {
            mockCheckIsWebview.mockReturnValue(false);
            // mockedIsAlfaMobileWebview = jest.fn(() => false);

            expect(detectAndExtractNativeParams({ headers: {}, query: {} })).toEqual({
                isWebview: false,
            });
        });

        it('should pass AM-webview env', () => {
            mockCheckIsWebview.mockReturnValue(true);
            // mockedIsAlfaMobileWebview = jest.fn(() => true);
    ...
    describe('originalWebviewParams', () => {
        ...
        it('should add correct originalWebviewParams', () => {
            expect(detectAndExtractNativeParams(request)).toEqual(returnValues);
        });
    });
    ...
    describe('addCookie', () => {
        const addCookieMock = jest.fn();
        const NATIVE_PARAMS_COOKIE_NAME = 'app_native_params';
        ...
        it('should use addCookie function to set cookie with nativeParams', () => {
            detectAndExtractNativeParams(request, addCookieMock);

            const encodedNativeParams = encodeURIComponent(JSON.stringify(returnValues));

            expect(addCookieMock).toHaveBeenCalledWith(
